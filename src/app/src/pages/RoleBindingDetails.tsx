...
export default function RoleBindingDetails() {
  const { cluster, namespace, roleBindingName } = useParams<{ cluster: string; namespace: string; roleBindingName: string }>()
  const navigate = useNavigate()
  const { addNotification } = useNotificationStore()
  const [activeTab, setActiveTab] = useState<TabType>('overview')
  const [roleBinding, setRoleBinding] = useState<any>(null)
  ...
  useEffect(() => {
    if (cluster && namespace && roleBindingName) {
      fetchRoleBindingDetails()
    }
  }, [cluster, namespace, roleBindingName])

  const fetchRoleBindingDetails = async () => {
    try {
      setIsLoading(true)
      const [rbRes, eventsRes] = await Promise.all([
        api.get(`/clusters/${cluster}/namespaces/${namespace}/rolebindings/${roleBindingName}`),
        api.get(`/clusters/${cluster}/events`).catch(() => ({ data: { events: [] } })),
      ])

      const rbData = rbRes.data
      const eventsData = eventsRes.data.events || []

      setRoleBinding(rbData)
      
      // Filter events related to this role binding
      const rbEvents = eventsData.filter((event: any) => {
        if (!roleBindingName) return false
        
        const involvedObjectMatch = event.involvedObject?.kind === 'RoleBinding' && 
                                    event.involvedObject?.name === roleBindingName &&
                                    event.involvedObject?.namespace === namespace
        const messageMatch = event.message?.toLowerCase().includes(roleBindingName.toLowerCase())
      ...
      const k8sManifest = {
        apiVersion: rbData.apiVersion || 'rbac.authorization.k8s.io/v1',
        kind: rbData.kind || 'RoleBinding',
        metadata: rbData.metadata,
        roleRef: rbData.roleRef,
        subjects: rbData.subjects,
      }
      setYamlContent(yaml.dump(k8sManifest, { indent: 2, lineWidth: -1, sortKeys: false }))
    } catch (error) {
      console.error('Failed to fetch role binding details:', error)
      addNotification({
        type: 'error',
        title: 'Error',
        message: 'Failed to load role binding details',
      })
    } finally {
      setIsLoading(false)
    }
  ...
  const handleSaveYaml = async () => {
    try {
      ...
      await api.put(`/clusters/${cluster}/namespaces/${namespace}/rolebindings/${roleBindingName}`, updatedManifest)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      addNotification({
        type: 'error',
        title: 'Error',
        message: `Failed to update role binding: ${error.message || 'Unknown error'}`,
      })
    }
  }
  ...
  const handleDeleteRoleBinding = async () => {
    try {
      await api.delete(`/clusters/${cluster}/namespaces/${namespace}/rolebindings/${roleBindingName}`)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      navigate(`/clusters/${cluster}/namespaces/${namespace}/rolebindings`)
    } catch (error: any) {
      addNotification({
        type: 'error',
        title: 'Error',
  ...
  if (!roleBinding) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 dark:text-gray-400">Role binding not found</p>
      </div>
    )
  }
    ...
    <div className="space-y-4 md:space-y-6">
      <Breadcrumb
        items={[
          ...
          { name: namespace || '', href: `/clusters/${cluster}/namespaces/${namespace}/rolebindings` },
          { name: 'Role Bindings', href: `/clusters/${cluster}/namespaces/${namespace}/rolebindings` },
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Role Binding Information</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Name:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{roleBinding.metadata.name}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Namespace:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{roleBinding.metadata.namespace}</p>
                </div>
                <div>
              ...
              {roleBinding.metadata.labels && Object.keys(roleBinding.metadata.labels).length > 0 && (
                <div className="mt-6">
                  ...
                  >
                    ...
                    Labels ({Object.keys(roleBinding.metadata.labels).length})
                  </button>
                  {expandedSections.labels && (
              ...
              {roleBinding.metadata.annotations && Object.keys(roleBinding.metadata.annotations).length > 0 && (
                <div className="mt-4">
                  ...
                  >
                    ...
                    Annotations ({Object.keys(roleBinding.metadata.annotations).length})
                  </button>
                  {expandedSections.annotations && (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{roleBinding.roleRef.kind}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Name:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{roleBinding.roleRef.name}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">API Group:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{roleBinding.roleRef.apiGroup || 'rbac.authorization.k8s.io'}</p>
                </div>
              </div>
            </div>

            {/* Subjects */}
            {roleBinding.subjects && roleBinding.subjects.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <button
                ...
                >
                  ...
                  Subjects ({roleBinding.subjects.length})
                </button>
                {expandedSections.subjects && (
                  <div className="mt-4 space-y-3">
                    {roleBinding.subjects.map((subject: any, index: number) => (
      ...
      {roleBinding && (
        <>
          <ConfirmationModal
            ...
            message="Are you sure you want to save the YAML changes? This will update the role binding."
            confirmText="Save"
            type="warning"
          />
          ...
          <ConfirmationModal
            ...
            message={`Are you sure you want to delete role binding "${roleBindingName}"? This action cannot be undone.`}
            confirmText="Delete"
            type="danger"
          />
        </>
