...
export default function ServiceAccountDetails() {
  const { cluster, namespace, serviceAccountName } = useParams<{ cluster: string; namespace: string; serviceAccountName: string }>()
  ...
  const fetchServiceAccountDetails = async () => {
    try {
      setIsLoading(true)
      const [saRes, eventsRes] = await Promise.all([
        api.get(`/clusters/${cluster}/namespaces/${namespace}/serviceaccounts/${serviceAccountName}`),
      ...
      setServiceAccount(saData)
      
      // Filter events related to this service account
      const saEvents = eventsData.filter((event: any) => {
        if (!serviceAccountName) return false
        
        const involvedObjectMatch = event.involvedObject?.kind === 'ServiceAccount' && 
                                    event.involvedObject?.name === serviceAccountName &&
                                    event.involvedObject?.namespace === namespace
        const messageMatch = event.message?.toLowerCase().includes(serviceAccountName.toLowerCase())
      ...
      const k8sManifest = {
        apiVersion: saData.apiVersion || 'v1',
        kind: saData.kind || 'ServiceAccount',
        metadata: saData.metadata,
        secrets: saData.secrets,
        imagePullSecrets: saData.imagePullSecrets,
        automountServiceAccountToken: saData.automountServiceAccountToken,
      }
      setYamlContent(yaml.dump(k8sManifest, { indent: 2, lineWidth: -1, sortKeys: false }))
    } catch (error) {
      console.error('Failed to fetch service account details:', error)
      addNotification({
        type: 'error',
        title: 'Error',
        message: 'Failed to load service account details',
      })
    } finally {
      setIsLoading(false)
    }
  ...
  const handleSaveYaml = async () => {
    try {
      ...
      await api.put(`/clusters/${cluster}/namespaces/${namespace}/serviceaccounts/${serviceAccountName}`, updatedManifest)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      addNotification({
        type: 'error',
        title: 'Error',
        message: `Failed to update service account: ${error.message || 'Unknown error'}`,
      })
    }
  }
  ...
  const handleDeleteServiceAccount = async () => {
    try {
      await api.delete(`/clusters/${cluster}/namespaces/${namespace}/serviceaccounts/${serviceAccountName}`)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      navigate(`/clusters/${cluster}/namespaces/${namespace}/serviceaccounts`)
    } catch (error: any) {
      addNotification({
        type: 'error',
        title: 'Error',
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Service Account Information</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Name:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{serviceAccount.metadata.name}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Namespace:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{serviceAccount.metadata.namespace}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Auto Mount Token:</span>
                ...
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Secrets:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
                    {(serviceAccount.secrets || []).length}
                  </p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Image Pull Secrets:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
              ...
              {serviceAccount.metadata.labels && Object.keys(serviceAccount.metadata.labels).length > 0 && (
                <div className="mt-6">
                  ...
                  >
                    ...
                    Labels ({Object.keys(serviceAccount.metadata.labels).length})
                  </button>
                  {expandedSections.labels && (
              ...
              {serviceAccount.metadata.annotations && Object.keys(serviceAccount.metadata.annotations).length > 0 && (
                <div className="mt-4">
                  ...
                  >
                    ...
                    Annotations ({Object.keys(serviceAccount.metadata.annotations).length})
                  </button>
                  {expandedSections.annotations && (
            ...
            {serviceAccount.secrets && serviceAccount.secrets.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                >
                  ...
                  Secrets ({serviceAccount.secrets.length})
                </button>
                {expandedSections.secrets && (
                  <div className="mt-4 space-y-2">
                    {serviceAccount.secrets.map((secret: any, index: number) => (
            ...
            {serviceAccount.imagePullSecrets && serviceAccount.imagePullSecrets.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <button
                  onClick={() => toggleSection('imagePullSecrets')}
                  className="flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400"
                >
                  {expandedSections.imagePullSecrets ? (
                    <ChevronUpIcon className="w-5 h-5" />
                  ) : (
                    <ChevronDownIcon className="w-5 h-5" />
                  )}
                  Image Pull Secrets ({serviceAccount.imagePullSecrets.length})
                </button>
                {expandedSections.imagePullSecrets && (
                  <div className="mt-4 space-y-2">
                    {serviceAccount.imagePullSecrets.map((secret: any, index: number) => (
                      <div key={index} className="p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
                        <span className="text-sm font-mono text-gray-900 dark:text-white">{secret.name}</span>
                      </div>
                    ))}
        ...
        {activeTab === 'events' && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <DataTable
              ...
              emptyMessage="No events found for this service account"
              pageSize={20}
            />
          </div>
        )}
      ...
      {serviceAccount && (
        <>
          <ConfirmationModal
            ...
            message="Are you sure you want to save the YAML changes? This will update the service account."
            confirmText="Save"
            type="warning"
          />
          ...
          <ConfirmationModal
            ...
            message={`Are you sure you want to delete service account "${serviceAccountName}"? This action cannot be undone.`}
            confirmText="Delete"
            type="danger"
          />
        </>
