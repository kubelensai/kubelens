...
export default function ReplicaSetDetails({}: ReplicaSetDetailsProps) {
  const { cluster, namespace, replicasetName } = useParams<{ cluster: string; namespace: string; replicasetName: string }>()
  const navigate = useNavigate()
  const { addNotification } = useNotificationStore()
  const [searchParams] = useSearchParams()
  const tabParam = searchParams.get('tab') as TabType | null
  const [activeTab, setActiveTab] = useState<TabType>(tabParam || 'overview')
  const [replicaset, setReplicaSet] = useState<any>(null)
  ...
  useEffect(() => {
    if (cluster && namespace && replicasetName) {
  ...
  useEffect(() => {
    if (activeTab !== 'pods' || !cluster || !namespace || !replicaset) return
    ...
    const refreshPods = async () => {
      try {
        ...
        // Filter pods by replicaset label selector
        const labelSelector = replicaset.spec?.selector?.matchLabels || {}
  ...
  }, [activeTab, cluster, namespace, replicaset])

  const fetchReplicaSetDetails = async () => {
    try {
      setIsLoading(true)
      const [replicasetRes, podsRes, eventsRes] = await Promise.all([
        api.get(`/clusters/${cluster}/namespaces/${namespace}/replicasets/${replicasetName}`),
        api.get(`/clusters/${cluster}/pods?namespace=${namespace}`).catch(() => ({ data: [] })),
        api.get(`/clusters/${cluster}/events`).catch(() => ({ data: { events: [] } })),
      ])

      const replicasetData = replicasetRes.data
      const podsData = podsRes.data || []
      const eventsData = eventsRes.data.events || []

      setReplicaSet(replicasetData)
      
      // Filter pods by replicaset label selector
      const labelSelector = replicasetData.spec?.selector?.matchLabels || {}
      const filteredPods = podsData.filter((pod: any) => {
        const podLabels = pod.metadata?.labels || {}
        return Object.entries(labelSelector).every(([key, value]) => podLabels[key] === value)
      })
      ...
      // Filter events related to this replicaset
      const replicasetEvents = eventsData.filter((event: any) => {
        if (!replicasetName) return false
        
        const involvedObjectMatch = event.involvedObject?.kind === 'ReplicaSet' && 
                                    event.involvedObject?.name === replicasetName &&
                                    event.involvedObject?.namespace === namespace
        const messageMatch = event.message?.toLowerCase().includes(replicasetName.toLowerCase())
        
        return involvedObjectMatch || messageMatch
      })
      
      setEvents(replicasetEvents)
      
      // Format as Kubernetes manifest
      const k8sManifest = {
        apiVersion: replicasetData.apiVersion || 'apps/v1',
        kind: replicasetData.kind || 'ReplicaSet',
        metadata: replicasetData.metadata,
        spec: replicasetData.spec,
        status: replicasetData.status,
      }
      setYamlContent(yaml.dump(k8sManifest, { indent: 2, lineWidth: -1, sortKeys: false }))
    } catch (error) {
      console.error('Failed to fetch replicaset details:', error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSaveYaml = async () => {
    try {
      ...
      await api.put(`/clusters/${cluster}/namespaces/${namespace}/replicasets/${replicasetName}`, updatedManifest)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      addNotification({
        type: 'error',
        title: 'Error',
        message: `Failed to update replicaset: ${errorMessage}`,
      })
    }
  }

  const handleRestartReplicaSet = async () => {
    try {
      await api.post(`/clusters/${cluster}/namespaces/${namespace}/replicasets/${replicasetName}/restart`)
      addNotification({
        type: 'success',
        title: 'Success',
        message: 'ReplicaSet restarted successfully',
      })
      fetchReplicaSetDetails()
      ...
      addNotification({
        type: 'error',
        title: 'Error',
        message: `Failed to restart replicaset: ${error.message || 'Unknown error'}`,
      })
    }
  }
  ...
  const handleDeleteReplicaSet = async () => {
    try {
      await api.delete(`/clusters/${cluster}/namespaces/${namespace}/replicasets/${replicasetName}`)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      navigate(`/clusters/${cluster}/namespaces/${namespace}/replicasets`)
    } catch (error: any) {
      addNotification({
        type: 'error',
        title: 'Error',
  ...
  const getReplicaSetStatus = () => {
    if (!replicaset) return 'Unknown'
    
    const desired = replicaset.spec.replicas || 0
    const ready = replicaset.status.readyReplicas || 0
    const current = replicaset.status.currentReplicas || 0
    const updated = replicaset.status.updatedReplicas || 0
  ...
  if (!replicaset) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 dark:text-gray-400">ReplicaSet not found</p>
      </div>
    )
  }
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">ReplicaSet Information</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Name:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{replicaset.metadata.name}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Namespace:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{replicaset.metadata.namespace}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Status:</span>
                  <p className={clsx('text-sm font-medium', getStatusColor(getReplicaSetStatus()))}>
                    {getReplicaSetStatus()}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
                    {replicaset.spec?.updateStrategy?.type || 'RollingUpdate'}
                  </p>
                </div>
                <div>
              ...
              {/* Labels */}
              {replicaset.metadata.labels && Object.keys(replicaset.metadata.labels).length > 0 && (
                <div className="mt-6">
                  <button
                  ...
                  >
                    ...
                    Labels ({Object.keys(replicaset.metadata.labels).length})
                  </button>
                  {expandedSections.labels && (
                    <div className="mt-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-200 dark:border-blue-800">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {Object.entries(replicaset.metadata.labels).map(([key, value]) => (
              ...
              {/* Annotations */}
              {replicaset.metadata.annotations && Object.keys(replicaset.metadata.annotations).length > 0 && (
                <div className="mt-4">
                  <button
                  ...
                  >
                    ...
                    Annotations ({Object.keys(replicaset.metadata.annotations).length})
                  </button>
                  {expandedSections.annotations && (
                    <div className="mt-3 p-4 bg-purple-50 dark:bg-purple-900/10 rounded-lg border border-purple-200 dark:border-purple-800 max-h-60 overflow-y-auto">
                      <div className="space-y-3">
                        {Object.entries(replicaset.metadata.annotations).map(([key, value]) => (
                          <div key={key} className="flex flex-col gap-1">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    {replicaset.status?.currentReplicas || 0}
                  </p>
                ...
                <div className="text-center p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    {replicaset.status?.updatedReplicas || 0}
                  </p>
        ...
        {activeTab === 'pods' && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <DataTable
              ...
              emptyMessage="No pods found for this replicaset"
              pageSize={20}
            />
          </div>
        )}
      ...
      {replicaset && (
        <>
          ...
          <ConfirmationModal
            ...
            message="Are you sure you want to save the YAML changes? This will update the replicaset."
            confirmText="Save"
            type="warning"
          />
          ...
          <ConfirmationModal
            ...
            onConfirm={handleRestartReplicaSet}
            title="Restart ReplicaSet"
            message={`Are you sure you want to restart replicaset "${replicasetName}"? This will restart all pods.`}
            confirmText="Restart"
            type="warning"
          />
          ...
          <ConfirmationModal
            ...
            onConfirm={handleDeleteReplicaSet}
            title="Delete ReplicaSet"
            message={`Are you sure you want to delete replicaset "${replicasetName}"? This action cannot be undone.`}
            confirmText="Delete"
            type="danger"
          />
