...
export default function PersistentVolumeClaimDetails({}: PersistentVolumeClaimDetailsProps) {
  const { cluster, namespace, pvcName } = useParams<{ cluster: string; namespace: string; pvcName: string }>()
  ...
  const fetchPVCDetails = async () => {
    try {
      setIsLoading(true)
      const [pvcRes, eventsRes] = await Promise.all([
        api.get(`/clusters/${cluster}/namespaces/${namespace}/persistentvolumeclaims/${pvcName}`),
        api.get(`/clusters/${cluster}/events`).catch(() => ({ data: { events: [] } })),
      ])

      const pvcData = pvcRes.data
      ...
      const k8sManifest = {
        apiVersion: pvcData.apiVersion || 'v1',
        kind: pvcData.kind || 'PersistentVolume',
  ...
  const handleSaveYaml = async () => {
    try {
      ...
      await api.put(`/clusters/${cluster}/namespaces/${namespace}/persistentvolumeclaims/${pvcName}`, updatedManifest)
      addNotification({
        type: 'success',
        title: 'Success',
  ...
  const handleDeletePVC = async () => {
    try {
      await api.delete(`/clusters/${cluster}/namespaces/${namespace}/persistentvolumeclaims/${pvcName}`)
      addNotification({
        type: 'success',
        title: 'Success',
      ...
      navigate(`/clusters/${cluster}/persistentvolumeclaims`)
    } catch (error: any) {
      addNotification({
        type: 'error',
        title: 'Error',
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Persistent Volume Claim Information</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Name:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{pvc.metadata.name}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Status:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{pvc.status?.phase || 'Unknown'}</p>
                </div>
                <div>
                ...
                <div>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Volume Name:</span>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
                    {pvc.spec?.volumeName || '-'}
                  </p>
                </div>
              ...
              {pvc.metadata.labels && Object.keys(pvc.metadata.labels).length > 0 && (
                <div className="mt-6">
                  ...
                  >
                    ...
                    Labels ({Object.keys(pvc.metadata.labels).length})
                  </button>
                  {expandedSections.labels && (
              ...
              {/* Annotations */}
              {pvc.metadata.annotations && Object.keys(pvc.metadata.annotations).length > 0 && (
                <div className="mt-4">
                  <button
                  ...
                  >
                    ...
                    Annotations ({Object.keys(pvc.metadata.annotations).length})
                  </button>
                  {expandedSections.annotations && (
                    <div className="mt-3 p-4 bg-purple-50 dark:bg-purple-900/10 rounded-lg border border-purple-200 dark:border-purple-800 max-h-60 overflow-y-auto">
                      <div className="space-y-3">
                        {Object.entries(pvc.metadata.annotations).map(([key, value]) => (
                          <div key={key} className="flex flex-col gap-1">
      ...
      {pvc && (
        <>
          ...
          <ConfirmationModal
            ...
            onConfirm={handleDeletePVC}
            title="Delete Persistent Volume Claim"
            message={`Are you sure you want to delete persistent volume "${pvcName}"? This action cannot be undone.`}
            confirmText="Delete"
            type="danger"
          />
        </>
