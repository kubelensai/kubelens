...
import { deleteNetworkPolicy } from '@/services/api'
import { notifyResourceAction } from '@/utils/notifications'

interface DeleteNetworkPolicyModalProps {
  networkPolicy: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function DeleteNetworkPolicyModal({
  networkPolicy,
  isOpen,
  onClose,
  onSuccess,
}: DeleteNetworkPolicyModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteNetworkPolicy(
        networkPolicy.clusterName,
        networkPolicy.metadata.namespace,
        networkPolicy.metadata.name
      )

      notifyResourceAction.deleted('Network Policy', networkPolicy.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to delete network policy'
      setError(errorMsg)
      notifyResourceAction.failed('delete', 'Network Policy', networkPolicy.metadata.name, errorMsg)
    } finally {
      setIsDeleting(false)
    }
  }
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="p-6">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Are you sure you want to delete this network policy? This action cannot be undone.
                    <span className="block mt-2 font-medium text-red-600 dark:text-red-400">
                      Warning: Deleting this policy may expose pods to network traffic.
                    </span>
                  </p>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white truncate max-w-[200px]" title={getPodSelectorDisplay()}>
                        {getPodSelectorDisplay()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
