...
interface EditNetworkPolicyYAMLModalProps {
  networkPolicy: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditNetworkPolicyYAMLModal({
  networkPolicy,
  isOpen,
  onClose,
  onSuccess,
}: EditNetworkPolicyYAMLModalProps) {
  const [networkPolicyYaml, setNetworkPolicyYaml] = useState('')
  const [error, setError] = useState('')
  const [isLoading, setIsLoading] = useState(true)
  const [isSaving, setIsSaving] = useState(false)

  useEffect(() => {
    if (isOpen && networkPolicy) {
      fetchNetworkPolicyYaml()
    }
  }, [isOpen, networkPolicy])

  const fetchNetworkPolicyYaml = async () => {
    setIsLoading(true)
    try {
      const response = await api.get(
        `/clusters/${networkPolicy.clusterName}/namespaces/${networkPolicy.metadata.namespace}/networkpolicies/${networkPolicy.metadata.name}`
      )
      const cleaned = cleanKubernetesManifest(response.data, 'networkpolicy')
      setNetworkPolicyYaml(yaml.dump(cleaned, { indent: 2, lineWidth: -1 }))
      setError('')
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to fetch network policy')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const parsed = yaml.load(networkPolicyYaml) as any
      ...
      notifyResourceAction.updated('Network Policy', networkPolicy.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to update network policy'
      setError(errorMsg)
      notifyResourceAction.failed('update', 'Network Policy', networkPolicy.metadata.name, errorMsg)
    } finally {
      setIsSaving(false)
    }
  }
                ...
                <div className="p-6 space-y-4">
                  ...
                  {isLoading ? (
                    ...
                    <div className="h-[60vh] border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                      <YamlEditor
                        value={networkPolicyYaml}
                        onChange={setNetworkPolicyYaml}
                      />
                    </div>
                  )}
                </div>
