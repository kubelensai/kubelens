...
import { createClusterRole } from '@/services/api'
import yaml from 'js-yaml'
import { useNotificationStore } from '@/stores/notificationStore'

interface CreateClusterRoleModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: () => void
  cluster: string
...
export default function CreateClusterRoleModal({
  isOpen,
  onClose,
  onSuccess,
  cluster,
}: CreateClusterRoleModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: ({ clusterName, yaml }: { clusterName: string; yaml: string }) =>
      createClusterRole(clusterName, yaml),
    onSuccess: () => {
      addNotification({
        type: 'success',
        title: 'Success',
        message: 'Cluster Role created successfully',
      })
      onSuccess?.()
      handleClose()
    },
    onError: (error: any) => {
      addNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.error || 'Failed to create cluster role',
      })
    },
  })
  ...
  const handleCreate = () => {
    if (!name.trim()) {
      addNotification({
        ...
        message: 'Cluster Role name is required',
      })
      return
    }

    // Build ClusterRole object
    const clusterRole: any = {
    ...
    if (labels.length > 0) {
      ...
      if (Object.keys(labelsObj).length > 0) {
        clusterRole.metadata.labels = labelsObj
      }
    }
    ...
    if (annotations.length > 0) {
      ...
      if (Object.keys(annotationsObj).length > 0) {
        clusterRole.metadata.annotations = annotationsObj
      }
    }

    const yamlString = yaml.dump(clusterRole)
