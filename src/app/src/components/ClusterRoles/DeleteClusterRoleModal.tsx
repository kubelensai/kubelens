...
import { deleteClusterRole } from '@/services/api'
import { useNotificationStore } from '@/stores/notificationStore'

interface DeleteClusterRoleModalProps {
  clusterRole: any
  isOpen: boolean
  onClose: () => void
  onSuccess?: () => void
...
export default function DeleteClusterRoleModal({
  clusterRole,
  isOpen,
  onClose,
  onSuccess,
}: DeleteClusterRoleModalProps) {
  const { addNotification } = useNotificationStore()

  const deleteMutation = useMutation({
    mutationFn: ({ clusterName, crName }: { clusterName: string; crName: string }) =>
      deleteClusterRole(clusterName, crName),
    onSuccess: () => {
      addNotification({
        type: 'success',
        title: 'Success',
    ...
    onError: (error: any) => {
      addNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.error || 'Failed to delete cluster role',
      })
    },
  })
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-gray-700 dark:text-gray-300 mb-4">
                        Are you sure you want to delete this cluster role? This action cannot be undone.
                      </p>
