...
export default function CronJobDetailsModal({ cronjob, isOpen, onClose }: CronJobDetailsModalProps) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl sm:p-6">
                ...
                <div className="sm:flex sm:items-start">
                  <div className="w-full mt-3 text-center sm:mt-0 sm:text-left">
                    ...
                    >
                      CronJob Details: {cronjob.metadata.name}
                    </Dialog.Title>
                    ...
                    <div className="mt-4 space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            <span className="ml-2 text-gray-900 dark:text-white">
                              {formatDistanceToNow(new Date(cronjob.metadata.creationTimestamp), {
                                addSuffix: true,
                              })}
                            </span>
                          </div>
                      ...
                      <div className="border-b border-gray-200 dark:border-gray-700 pb-4">
                        ...
                        <div className="text-sm space-y-2">
                          <div>
                            ...
                            <span className="ml-2 text-gray-900 dark:text-white font-medium">
                              {cronjob.spec.strategy?.type || 'RollingUpdate'}
                            </span>
                          </div>
                          {cronjob.spec.strategy?.rollingUpdate && (
                            <>
                              <div>
                                <span className="text-gray-500 dark:text-gray-400">Max Surge:</span>
                                <span className="ml-2 text-gray-900 dark:text-white">
                                  {cronjob.spec.strategy.rollingUpdate.maxSurge || 'N/A'}
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-500 dark:text-gray-400">Max Unavailable:</span>
                                <span className="ml-2 text-gray-900 dark:text-white">
                                  {cronjob.spec.strategy.rollingUpdate.maxUnavailable || 'N/A'}
                                </span>
                              </div>
                            </>
                          )}
                      ...
                      {/* Selector */}
                      {cronjob.spec.selector?.matchLabels && (
                        <div className="border-b border-gray-200 dark:border-gray-700 pb-4">
                          <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">
                            Selector
                          </h4>
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(cronjob.spec.selector.matchLabels).map(([key, value]) => (
                              <span
                                key={key}
                      ...
                      {/* Conditions */}
                      {cronjob.status.conditions && cronjob.status.conditions.length > 0 && (
                        <div>
                          <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">
                            Conditions
                          </h4>
                          <div className="space-y-2">
                            {cronjob.status.conditions.map((condition: any, index: number) => (
                              <div
                                key={index}
