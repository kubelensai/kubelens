...
export default function EditCronJobModal({
  ...
  const fetchCronJobYaml = async () => {
    setIsLoading(true)
    setError('')
    try {
      const response = await api.get(
        `/clusters/${cronjob.clusterName}/namespaces/${cronjob.metadata.namespace}/cronjobs/${cronjob.metadata.name}`
      )
      // Clean and reorder the manifest
      const cleanedManifest = cleanKubernetesManifest(response.data, 'cronjob')
      ...
      setCronJobYaml(yamlStr)
    } catch (err) {
      console.error('Failed to fetch cronjob:', err)
      setError('Failed to load cronjob data')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      // Parse YAML to JSON
      const parsedCronJob = yaml.load(cronjobYaml)
      ...
      if (err.name === 'YAMLException') {
        ...
        setError(
          err.response?.data?.error || 'Failed to update cronjob. Please check your YAML syntax.'
        )
      }
    } finally {
      setIsSaving(false)
              ...
              <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Edit CronJob: {cronjob.metadata?.name}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
