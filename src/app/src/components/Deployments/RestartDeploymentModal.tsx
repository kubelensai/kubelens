...
interface RestartDeploymentModalProps {
  deployment: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function RestartDeploymentModal({
  deployment,
  isOpen,
  onClose,
  onSuccess,
}: RestartDeploymentModalProps) {
  const restartMutation = useMutation({
    mutationFn: async () => {
      await api.post(
        `/clusters/${deployment.clusterName}/namespaces/${deployment.metadata.namespace}/deployments/${deployment.metadata.name}/restart`
      )
    },
    onSuccess: () => {
      notifyResourceAction.restarted('Deployment', deployment.metadata.name)
      onSuccess()
    },
    onError: (err: any) => {
      console.error('Failed to restart deployment:', err)
      notifyResourceAction.failed('restart', 'Deployment', deployment.metadata.name, err.response?.data?.error || err.message)
    },
  })
                ...
                {restartMutation.isError && (
                  <div className="mt-4 rounded-md bg-red-50 dark:bg-red-900/20 p-3">
                    <p className="text-sm text-red-800 dark:text-red-300">
                      Failed to restart deployment. Please try again.
                    </p>
                  </div>
                )}
              </Dialog.Panel>
