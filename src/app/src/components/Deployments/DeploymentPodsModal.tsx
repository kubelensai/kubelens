...
interface DeploymentPodsModalProps {
  deployment: any
  isOpen: boolean
  onClose: () => void
}
...
export default function DeploymentPodsModal({
  deployment,
  isOpen,
  onClose,
}: DeploymentPodsModalProps) {
  ...
  const { data: allPods, isLoading } = useQuery({
    queryKey: ['deployment-pods', deployment?.clusterName, deployment?.metadata?.namespace, deployment?.metadata?.name],
    queryFn: async () => {
      if (!deployment) return []
      const pods = await getPods(deployment.clusterName, deployment.metadata.namespace)
      return pods
    },
  ...
  useEffect(() => {
    if (allPods && deployment) {
      const deploymentSelector = deployment.spec?.selector?.matchLabels || {}
      const filteredPods = allPods.filter((pod: any) => {
        const podLabels = pod.metadata?.labels || {}
        // Check if all deployment selector labels match pod labels
        return Object.entries(deploymentSelector).every(
  ...
  useEffect(() => {
    if (isOpen && pods.length > 0 && deployment) {
      fetchPodMetrics()
    }
      ...
      const metricsPromises = pods.map(async (pod) => {
        try {
          const response = await api.get(
            `/clusters/${deployment.clusterName}/namespaces/${pod.metadata.namespace}/pods/${pod.metadata.name}/metrics`
          )
          return {
  ...
  const handlePodClick = (_pod: any) => {
    navigate(`/clusters/${deployment.clusterName}/namespaces/${deployment.metadata.namespace}/pods`)
    onClose()
  }
                  ...
                  {isLoading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        This deployment has no running pods
                      </p>
                    </div>
                  )}
                </div>
