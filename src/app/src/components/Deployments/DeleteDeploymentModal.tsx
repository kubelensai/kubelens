...
export default function DeleteDeploymentModal({
  ...
  if (!deployment) {
    console.error('DeleteDeploymentModal: deployment is null!')
    return null
  }
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      notifyResourceAction.deleted('Deployment', deployment.metadata.name)
      onSuccess()
    },
    onError: (err: any) => {
      console.error('Failed to delete deployment:', err)
      notifyResourceAction.failed('delete', 'Deployment', deployment.metadata.name, err.response?.data?.error || err.message)
    },
  })
                          ...
                          <div className="ml-3">
                            ...
                            <div className="mt-2 text-sm text-red-700 dark:text-red-400">
                              <ul className="list-disc list-inside space-y-1">
                                <li>All pods managed by this deployment will be deleted</li>
                ...
                {deleteMutation.isError && (
                  <div className="mt-4 rounded-md bg-red-50 dark:bg-red-900/20 p-3">
                    <p className="text-sm text-red-800 dark:text-red-300">
                      Failed to delete deployment. Please try again.
                    </p>
                  </div>
                )}
              </Dialog.Panel>
