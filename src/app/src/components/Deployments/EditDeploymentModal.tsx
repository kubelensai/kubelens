...
export default function EditDeploymentModal({
  ...
  const fetchDeploymentYaml = async () => {
    setIsLoading(true)
    setError('')
    try {
      const response = await api.get(
        `/clusters/${deployment.clusterName}/namespaces/${deployment.metadata.namespace}/deployments/${deployment.metadata.name}`
      )
      
      // Clean and reorder the manifest
      const cleanedManifest = cleanKubernetesManifest(response.data, 'deployment')
  ...
  const handleSave = async () => {
    ...
    try {
      // Parse YAML to JSON
      const parsedDeployment = yaml.load(deploymentYaml)
      ...
      notifyResourceAction.updated('Deployment', deployment.metadata.name)
      onSuccess()
    } catch (err: any) {
      console.error('Failed to update deployment:', err)
      if (err.name === 'YAMLException') {
        setError(`YAML Syntax Error: ${err.message}`)
        notifyResourceAction.failed('update', 'Deployment', deployment.metadata.name, `YAML Syntax Error: ${err.message}`)
      } else {
        const errorMsg = err.response?.data?.error || 'Failed to update deployment. Please check your YAML syntax.'
        setError(errorMsg)
        notifyResourceAction.failed('update', 'Deployment', deployment.metadata.name, errorMsg)
      }
    } finally {
      setIsSaving(false)
    }
              ...
              <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Edit Deployment: {deployment.metadata?.name}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
