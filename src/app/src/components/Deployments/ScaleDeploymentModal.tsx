...
export default function ScaleDeploymentModal({
  deployment,
  isOpen,
  onClose,
  onSuccess,
  ...
  // Update replicas when deployment changes or modal opens
  useEffect(() => {
  ...
  const scaleMutation = useMutation({
    mutationFn: async (newReplicas: number) => {
      await api.patch(
        `/clusters/${deployment.clusterName}/namespaces/${deployment.metadata.namespace}/deployments/${deployment.metadata.name}/scale`,
        { replicas: newReplicas }
      )
      return newReplicas
    },
    onSuccess: (scaledReplicas) => {
      notifyResourceAction.scaled('Deployment', deployment.metadata.name, scaledReplicas)
      onSuccess()
    },
    onError: (err: any) => {
      const errorMsg = err.response?.data?.error || 'Failed to scale deployment'
      setError(errorMsg)
      notifyResourceAction.failed('scale', 'Deployment', deployment.metadata.name, errorMsg)
    },
  })
