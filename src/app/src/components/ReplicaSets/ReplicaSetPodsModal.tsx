...
interface ReplicaSetPodsModalProps {
  replicaset: any
  isOpen: boolean
  onClose: () => void
}
...
export default function ReplicaSetPodsModal({
  replicaset,
  isOpen,
  onClose,
}: ReplicaSetPodsModalProps) {
  ...
  const { data: allPods, isLoading } = useQuery({
    queryKey: ['replicaset-pods', replicaset?.clusterName, replicaset?.metadata?.namespace, replicaset?.metadata?.name],
    queryFn: async () => {
      if (!replicaset) return []
      const pods = await getPods(replicaset.clusterName, replicaset.metadata.namespace)
      return pods
    },
    enabled: isOpen && !!replicaset,
  })

  // Filter pods that belong to this replicaset
  useEffect(() => {
    if (allPods && replicaset) {
      const replicasetSelector = replicaset.spec?.selector?.matchLabels || {}
      const filteredPods = allPods.filter((pod: any) => {
        const podLabels = pod.metadata?.labels || {}
        // Check if all replicaset selector labels match pod labels
        return Object.entries(replicasetSelector).every(
  ...
  useEffect(() => {
    if (isOpen && pods.length > 0 && replicaset) {
      fetchPodMetrics()
    }
      ...
      const metricsPromises = pods.map(async (pod) => {
        try {
          const response = await api.get(
            `/clusters/${replicaset.clusterName}/namespaces/${pod.metadata.namespace}/pods/${pod.metadata.name}/metrics`
          )
          return {
  ...
  const handlePodClick = (_pod: any) => {
    navigate(`/clusters/${replicaset.clusterName}/namespaces/${replicaset.metadata.namespace}/pods`)
    onClose()
  }
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                      {replicaset.metadata.name} • {replicaset.metadata.namespace}
                    </p>
                  </div>
                  <button
                    onClick={onClose}
                  ...
                  {isLoading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        This replicaset has no running pods
                      </p>
                    </div>
                  )}
                </div>
