...
interface EditReplicaSetModalProps {
  replicaset: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditReplicaSetModal({
  replicaset,
  isOpen,
  onClose,
  onSuccess,
}: EditReplicaSetModalProps) {
  const [replicasetYaml, setReplicaSetYaml] = useState<string>('')
  const [isLoading, setIsLoading] = useState(false)
  const [isSaving, setIsSaving] = useState(false)
  const [error, setError] = useState<string>('')

  useEffect(() => {
    if (isOpen && replicaset) {
      fetchReplicaSetYaml()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, replicaset])

  const fetchReplicaSetYaml = async () => {
    setIsLoading(true)
    setError('')
    try {
      const response = await api.get(
        `/clusters/${replicaset.clusterName}/namespaces/${replicaset.metadata.namespace}/replicasets/${replicaset.metadata.name}`
      )
      // Clean and reorder the manifest
      const cleanedManifest = cleanKubernetesManifest(response.data, 'replicaset')
      ...
      setReplicaSetYaml(yamlStr)
    } catch (err) {
      console.error('Failed to fetch replicaset:', err)
      setError('Failed to load replicaset data')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      // Parse YAML to JSON
      const parsedReplicaSet = yaml.load(replicasetYaml)
      ...
      console.error('Failed to update replicaset:', err)
      if (err.name === 'YAMLException') {
        setError(`YAML Syntax Error: ${err.message}`)
      } else {
        setError(
          err.response?.data?.error || 'Failed to update replicaset. Please check your YAML syntax.'
        )
      }
    } finally {
      setIsSaving(false)
              ...
              <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Edit ReplicaSet: {replicaset.metadata?.name}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
                  ...
                  {isLoading ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <YamlEditor
                          value={replicasetYaml}
                          onChange={setReplicaSetYaml}
                          height="500px"
                        />
                      </div>
                    </div>
