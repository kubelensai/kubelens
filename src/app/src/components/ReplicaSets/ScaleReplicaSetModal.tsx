...
interface ScaleReplicaSetModalProps {
  replicaset: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function ScaleReplicaSetModal({
  replicaset,
  isOpen,
  onClose,
  onSuccess,
}: ScaleReplicaSetModalProps) {
  const [replicas, setReplicas] = useState<number>(replicaset?.spec?.replicas || 1)
  const [error, setError] = useState<string>('')

  // Update replicas when replicaset changes or modal opens
  useEffect(() => {
    if (isOpen && replicaset) {
      setReplicas(replicaset.spec?.replicas || 1)
      setError('')
    }
  }, [isOpen, replicaset])
  ...
  const scaleMutation = useMutation({
    mutationFn: async (newReplicas: number) => {
      await api.patch(
        `/clusters/${replicaset.clusterName}/namespaces/${replicaset.metadata.namespace}/replicasets/${replicaset.metadata.name}/scale`,
    ...
    onError: (err: any) => {
      setError(err.response?.data?.error || 'Failed to scale replicaset')
    },
  })
                    ...
                    <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                      <div>
                        ...
                        <input
                          type="text"
                          id="replicaset-name"
                          value={replicaset?.metadata?.name || ''}
                          disabled
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          id="current-replicas"
                          value={replicaset?.spec?.replicas || 0}
                          disabled
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed"
                        />
                      </div>
