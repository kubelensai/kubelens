...
interface ReplicaSetDetailsModalProps {
  replicaset: any
  isOpen: boolean
  onClose: () => void
}

export default function ReplicaSetDetailsModal({ replicaset, isOpen, onClose }: ReplicaSetDetailsModalProps) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl sm:p-6">
                ...
                <div className="sm:flex sm:items-start">
                  <div className="w-full mt-3 text-center sm:mt-0 sm:text-left">
                    ...
                    >
                      ReplicaSet Details: {replicaset.metadata.name}
                    </Dialog.Title>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            <span className="ml-2 text-gray-900 dark:text-white">
                              {formatDistanceToNow(new Date(replicaset.metadata.creationTimestamp), {
                                addSuffix: true,
                              })}
                            </span>
                          </div>
                      ...
                      <div className="border-b border-gray-200 dark:border-gray-700 pb-4">
                        ...
                        <div className="text-sm space-y-2">
                          <div>
                            ...
                            <span className="ml-2 text-gray-900 dark:text-white font-medium">
                              {replicaset.spec.strategy?.type || 'RollingUpdate'}
                            </span>
                          </div>
                          {replicaset.spec.strategy?.rollingUpdate && (
                            <>
                              <div>
                                <span className="text-gray-500 dark:text-gray-400">Max Surge:</span>
                                <span className="ml-2 text-gray-900 dark:text-white">
                                  {replicaset.spec.strategy.rollingUpdate.maxSurge || 'N/A'}
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-500 dark:text-gray-400">Max Unavailable:</span>
                                <span className="ml-2 text-gray-900 dark:text-white">
                                  {replicaset.spec.strategy.rollingUpdate.maxUnavailable || 'N/A'}
                                </span>
                              </div>
                            </>
                          )}
                      ...
                      {replicaset.status.conditions && replicaset.status.conditions.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-2">
                            {replicaset.status.conditions.map((condition: any, index: number) => (
                              <div
                                key={index}
