...
import { getRuntimeClass } from '@/services/api'
import yaml from 'js-yaml'
import { cleanKubernetesManifest } from '@/utils/kubernetes'
import api from '@/services/api'
import YamlEditor from '@/components/shared/YamlEditor'

interface EditRuntimeClassYAMLModalProps {
  runtimeClass: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditRuntimeClassYAMLModal({
  runtimeClass,
  isOpen,
  onClose,
  onSuccess,
}: EditRuntimeClassYAMLModalProps) {
  ...
  useEffect(() => {
    if (isOpen && runtimeClass) {
      setIsLoading(true)
      setError(null)
      
      getRuntimeClass(runtimeClass.clusterName, runtimeClass.metadata.name)
        .then((data) => {
          const cleanedManifest = cleanKubernetesManifest(data, 'runtimeclass')
        ...
        .catch((err) => {
          console.error('Failed to fetch runtime class:', err)
          setError('Failed to load runtime class YAML')
          setIsLoading(false)
        })
    }
  }, [isOpen, runtimeClass])
  ...
  const handleSave = async () => {
    try {
      ...
      await api.put(
        `/clusters/${runtimeClass.clusterName}/runtimeclasses/${runtimeClass.metadata.name}`,
        parsedYaml
      )
      ...
      console.error('Failed to update runtime class:', err)
      setError(err.response?.data?.error || err.message || 'Failed to update runtime class')
    } finally {
      setIsSaving(false)
    }
  }
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                      <CommandLineIcon className="h-6 w-6 text-yellow-600 dark:text-yellow-400" />
                    </div>
                    <div>
                      <Dialog.Title className="text-xl font-semibold text-gray-900 dark:text-white">
                        Edit Runtime Class YAML
                      </Dialog.Title>
