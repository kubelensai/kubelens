...
import { deleteRuntimeClass } from '@/services/api'

interface DeleteRuntimeClassModalProps {
  runtimeClass: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function DeleteRuntimeClassModal({
  runtimeClass,
  isOpen,
  onClose,
  onSuccess,
}: DeleteRuntimeClassModalProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      await deleteRuntimeClass(runtimeClass.clusterName, runtimeClass.metadata.name)

      onSuccess()
      onClose()
    } catch (err: any) {
      console.error('Failed to delete runtime class:', err)
      setError(err.response?.data?.error || err.message || 'Failed to delete runtime class')
    } finally {
      setIsDeleting(false)
    }
  }
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 shadow-xl transition-all">
                ...
                <div className="px-6 py-4">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 mb-4">
                    Are you sure you want to delete the runtime class{' '}
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                      <strong>Warning:</strong> Deleting this runtime class will affect all pods
                      that reference it. Ensure no pods are using this runtime class before deletion.
                    </p>
                  </div>
                </div>
