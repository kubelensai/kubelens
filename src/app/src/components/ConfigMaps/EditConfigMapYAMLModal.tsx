...
interface EditConfigMapYAMLModalProps {
  configMap: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditConfigMapYAMLModal({ configMap, isOpen, onClose, onSuccess }: EditConfigMapYAMLModalProps) {
  ...
  const fetchConfigMapYaml = async () => {
    setIsLoading(true)
    try {
      const response = await api.get(
        `/clusters/${configMap.clusterName}/namespaces/${configMap.metadata.namespace}/configmaps/${configMap.metadata.name}`
      )
      const cleaned = cleanKubernetesManifest(response.data, 'configmap')
      setConfigMapYaml(yaml.dump(cleaned, { indent: 2, lineWidth: -1 }))
      setError('')
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to fetch configmap')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const parsed = yaml.load(configMapYaml) as any
      ...
      const errorMsg = err.response?.data?.error || err.message || 'Failed to update configmap'
      setError(errorMsg)
              ...
              <Dialog.Panel className="w-full max-w-5xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Edit ConfigMap YAML: {configMap?.metadata?.name}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
