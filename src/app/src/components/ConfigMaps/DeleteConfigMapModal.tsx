...
import { deleteConfigMap } from '@/services/api'
import { notifyResourceAction } from '@/utils/notifications'

interface DeleteConfigMapModalProps {
  configMap: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function DeleteConfigMapModal({ configMap, isOpen, onClose, onSuccess }: DeleteConfigMapModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notifyResourceAction.deleted('ConfigMap', configMap.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to delete configmap'
      setError(errorMsg)
      notifyResourceAction.failed('delete', 'ConfigMap', configMap.metadata.name, errorMsg)
    } finally {
      setIsDeleting(false)
    }
  }
                ...
                <div className="p-6">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">
                        {configMap.data ? Object.keys(configMap.data).length : 0} keys
                      </span>
                    </div>
                  </div>
