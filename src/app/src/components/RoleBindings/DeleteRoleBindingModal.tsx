...
import { deleteRoleBinding } from '@/services/api'
import { useNotificationStore } from '@/stores/notificationStore'

interface DeleteRoleBindingModalProps {
  roleBinding: any
  isOpen: boolean
  onClose: () => void
  onSuccess?: () => void
...
export default function DeleteRoleBindingModal({
  roleBinding,
  isOpen,
  onClose,
  onSuccess,
}: DeleteRoleBindingModalProps) {
  const { addNotification } = useNotificationStore()

  const deleteMutation = useMutation({
    mutationFn: ({ clusterName, namespace, roleName }: { clusterName: string; namespace: string; roleName: string }) =>
      deleteRoleBinding(clusterName, namespace, roleName),
    onSuccess: () => {
      addNotification({
        type: 'success',
        title: 'Success',
    ...
    onError: (error: any) => {
      addNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.error || 'Failed to delete role binding',
      })
    },
  })

  const handleDelete = () => {
    const clusterName = roleBinding.ClusterName
    const namespace = roleBinding.metadata?.namespace
    const roleName = roleBinding.metadata?.name
    deleteMutation.mutate({ clusterName, namespace, roleName })
  }
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-gray-700 dark:text-gray-300 mb-4">
                        Are you sure you want to delete this role binding? This action cannot be undone.
                      </p>
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          <span className="font-medium text-gray-500 dark:text-gray-400">Namespace:</span>
                          <span className="text-gray-900 dark:text-white font-mono">
                        ...
                        <div className="flex justify-between text-sm">
                          <span className="font-medium text-gray-500 dark:text-gray-400">Bindings:</span>
