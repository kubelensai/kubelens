...
import { getRoleBinding, updateRoleBinding } from '@/services/api'
import { cleanKubernetesManifest } from '@/utils/kubernetes'
import yaml from 'js-yaml'
import YamlEditor from '@/components/shared/YamlEditor'
import { useNotificationStore } from '@/stores/notificationStore'

interface EditRoleBindingYAMLModalProps {
  roleBinding: any
  isOpen: boolean
  onClose: () => void
  onSuccess?: () => void
...
export default function EditRoleBindingYAMLModal({
  roleBinding,
  isOpen,
  onClose,
  onSuccess,
}: EditRoleBindingYAMLModalProps) {
  ...
  useEffect(() => {
    if (isOpen && roleBinding) {
      ...
      getRoleBinding(clusterName, namespace, roleName)
        .then((data) => {
          const cleaned = cleanKubernetesManifest(data, 'rolebinding')
          const manifest = yaml.dump(cleaned, { indent: 2, lineWidth: -1 })
          setYamlContent(manifest)
        })
        .catch((error) => {
          console.error('Failed to fetch roleBinding:', error)
          addNotification({
            type: 'error',
            title: 'Error',
            message: 'Failed to load roleBinding YAML',
          })
        })
        .finally(() => {
  ...
  }, [isOpen, roleBinding, addNotification])
  ...
  const updateMutation = useMutation({
    mutationFn: ({ clusterName, namespace, roleName, yamlContent }: { clusterName: string; namespace: string; roleName: string; yamlContent: string }) =>
      updateRoleBinding(clusterName, namespace, roleName, yamlContent),
    onSuccess: () => {
      addNotification({
        type: 'success',
        title: 'Success',
