...
import { getClusterRoleBinding, updateClusterRoleBinding } from '@/services/api'
import { cleanKubernetesManifest } from '@/utils/kubernetes'
import yaml from 'js-yaml'
import YamlEditor from '@/components/shared/YamlEditor'
import { useNotificationStore } from '@/stores/notificationStore'

interface EditClusterRoleBindingYAMLModalProps {
...
export default function EditClusterRoleBindingYAMLModal({
  clusterRoleBinding,
  isOpen,
  onClose,
  onSuccess,
}: EditClusterRoleBindingYAMLModalProps) {
  const [yamlContent, setYamlContent] = useState('')
  const [isLoadingYaml, setIsLoadingYaml] = useState(false)
  const { addNotification } = useNotificationStore()

  useEffect(() => {
    if (isOpen && clusterRoleBinding) {
      setIsLoadingYaml(true)
      const clusterName = clusterRoleBinding.ClusterName
      const crName = clusterRoleBinding.metadata?.name

      getClusterRoleBinding(clusterName, crName)
        .then((data) => {
          const cleaned = cleanKubernetesManifest(data, 'clusterrolebinding')
          const manifest = yaml.dump(cleaned, { indent: 2, lineWidth: -1 })
          setYamlContent(manifest)
        })
        .catch((error) => {
          console.error('Failed to fetch cluster role binding:', error)
          addNotification({
            type: 'error',
            title: 'Error',
            message: 'Failed to load cluster role binding YAML',
          })
        })
        .finally(() => {
              ...
              <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Edit Cluster Role Binding YAML
                  </Dialog.Title>
                  <button
                    onClick={onClose}
