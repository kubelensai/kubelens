...
import { createClusterRoleBinding } from '@/services/api'
import yaml from 'js-yaml'
import { useNotificationStore } from '@/stores/notificationStore'

interface CreateClusterRoleBindingModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: () => void
  cluster: string
...
export default function CreateClusterRoleBindingModal({
  ...
  onSuccess,
  cluster,
}: CreateClusterRoleBindingModalProps) {
  const [name, setName] = useState('')
  const [roleRefKind, setRoleRefKind] = useState('ClusterRole')
  ...
  const createMutation = useMutation({
    mutationFn: ({ clusterName, yaml }: { clusterName: string; yaml: string }) =>
      createClusterRoleBinding(clusterName, yaml),
    onSuccess: () => {
      addNotification({
        type: 'success',
        title: 'Success',
        message: 'Cluster Role Binding created successfully',
      })
      onSuccess?.()
      handleClose()
    },
    onError: (error: any) => {
      addNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.error || 'Failed to create cluster role binding',
      })
    },
  })
  ...
  const handleCreate = () => {
    if (!name.trim()) {
      addNotification({
        ...
        message: 'Cluster Role Binding name is required',
      })
      return
    }
    ...
    // Build ClusterRoleBinding object
    const clusterRoleBinding: any = {
    ...
    if (labels.length > 0) {
      ...
      if (Object.keys(labelsObj).length > 0) {
        clusterRoleBinding.metadata.labels = labelsObj
      }
    }
    ...
    if (annotations.length > 0) {
      ...
      if (Object.keys(annotationsObj).length > 0) {
        clusterRoleBinding.metadata.annotations = annotationsObj
      }
    }

    const yamlString = yaml.dump(clusterRoleBinding)
    createMutation.mutate({ clusterName: cluster, yaml: yamlString })
  }
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., read-pods-global"
