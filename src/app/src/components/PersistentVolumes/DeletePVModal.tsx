...
import { deletePersistentVolume } from '@/services/api'
import { notifyResourceAction } from '@/utils/notifications'

interface DeletePVModalProps {
  pv: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function DeletePVModal({
  pv,
  isOpen,
  onClose,
  onSuccess,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePersistentVolume(
        pv.clusterName,
        pv.metadata.name
      )

      notifyResourceAction.deleted('PV', pv.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to delete PV'
      setError(errorMsg)
      notifyResourceAction.failed('delete', 'PV', pv.metadata.name, errorMsg)
    } finally {
      setIsDeleting(false)
    }
  }
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="p-6">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Are you sure you want to delete this persistent volume? This action cannot be undone.
                    <span className="block mt-2 font-medium text-red-600 dark:text-red-400">
                      Warning: Deleting this PV may result in data loss!
                    </span>
                  </p>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                    ...
                    {pv.spec?.claimRef && (
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 dark:text-gray-400">Bound to:</span>
                        <span className="font-medium text-gray-900 dark:text-white truncate max-w-[200px]" title={`${pv.spec.claimRef.namespace}/${pv.spec.claimRef.name}`}>
