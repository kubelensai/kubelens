...
interface EditPVYAMLModalProps {
  pv: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditPVYAMLModal({
  pv,
  isOpen,
  onClose,
  onSuccess,
}: EditPVYAMLModalProps) {
  const [pvYaml, setPvYaml] = useState('')
  ...
  const fetchPVYaml = async () => {
    setIsLoading(true)
    try {
      const response = await api.get(
        `/clusters/${pv.clusterName}/persistentvolumes/${pv.metadata.name}`
      )
      const cleaned = cleanKubernetesManifest(response.data, 'persistentvolume')
      setPvYaml(yaml.dump(cleaned, { indent: 2, lineWidth: -1 }))
      setError('')
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to fetch PV')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const parsed = yaml.load(pvYaml) as any
      ...
      notifyResourceAction.updated('PV', pv.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to update PV'
      setError(errorMsg)
      notifyResourceAction.failed('update', 'PV', pv.metadata.name, errorMsg)
    } finally {
      setIsSaving(false)
    }
  }
