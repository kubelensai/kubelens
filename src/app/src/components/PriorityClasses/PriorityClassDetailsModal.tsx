...
interface PriorityClassDetailsModalProps {
  priorityClass: any
  isOpen: boolean
  onClose: () => void
}

export default function PriorityClassDetailsModal({ priorityClass, isOpen, onClose }: PriorityClassDetailsModalProps) {
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900 dark:text-white">
                            {new Date(priorityClass.metadata?.creationTimestamp).toLocaleString()}
                          </p>
                        </div>
                        <div>
                          <span className="text-xs text-gray-500 dark:text-gray-400">UID</span>
                          <p className="text-xs font-mono text-gray-600 dark:text-gray-400 truncate">
                            {priorityClass.metadata?.uid}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-3">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <p className="text-lg font-bold text-gray-900 dark:text-white">{priorityClass.value}</p>
                        </div>
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-white mt-1">
                          {priorityClass.globalDefault ? (
                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                              Yes - This is the default priority for pods without a priority class
                            </span>
                          ) : (
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          {priorityClass.preemptionPolicy === 'Never'
                            ...
                            : 'Pods with this priority class can preempt pods with lower priority'}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {priorityClass.description && (
                    <div>
                      ...
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
                        <p className="text-sm text-gray-700 dark:text-gray-300">{priorityClass.description}</p>
                      </div>
                    </div>
                  )}
