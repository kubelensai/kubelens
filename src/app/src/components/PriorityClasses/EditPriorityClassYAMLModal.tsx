...
interface EditPriorityClassYAMLModalProps {
  priorityClass: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditPriorityClassYAMLModal({ priorityClass, isOpen, onClose, onSuccess }: EditPriorityClassYAMLModalProps) {
  const [yamlContent, setYamlContent] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState('')

  useEffect(() => {
    if (priorityClass && isOpen) {
      setIsLoading(true)
      api
        .get(`/clusters/${priorityClass.clusterName}/priorityclasses/${priorityClass.metadata?.name}`)
        .then((response) => {
          const cleanedPC = cleanKubernetesManifest(response.data, 'priorityclass')
          setYamlContent(yaml.dump(cleanedPC))
          setError('')
        })
        .catch((err) => {
          setError(err.message || 'Failed to load priority class YAML')
        })
        .finally(() => {
          setIsLoading(false)
        })
    }
  }, [priorityClass, isOpen])
  ...
  const handleSave = async () => {
    try {
      ...
      await api.put(
        `/clusters/${priorityClass.clusterName}/priorityclasses/${priorityClass.metadata?.name}`,
        parsed
      )

      notifyResourceAction.updated('Priority Class', priorityClass.metadata?.name)
      onSuccess()
      onClose()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to update priority class'
      setError(errorMsg)
      notifyResourceAction.failed('update', 'Priority Class', priorityClass.metadata?.name, errorMsg)
    } finally {
      setIsLoading(false)
    }
  }
