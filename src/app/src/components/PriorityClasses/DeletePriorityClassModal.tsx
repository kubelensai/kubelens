...
import { deletePriorityClass } from '@/services/api'
import { notifyResourceAction } from '@/utils/notifications'

interface DeletePriorityClassModalProps {
  priorityClass: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function DeletePriorityClassModal({ priorityClass, isOpen, onClose, onSuccess }: DeletePriorityClassModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePriorityClass(priorityClass.clusterName, priorityClass.metadata?.name)
      notifyResourceAction.deleted('Priority Class', priorityClass.metadata?.name)
      onSuccess()
      onClose()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to delete priority class'
      setError(errorMsg)
      notifyResourceAction.failed('delete', 'Priority Class', priorityClass.metadata?.name, errorMsg)
    } finally {
      setIsDeleting(false)
    }
  }

  if (!priorityClass) return null
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Are you sure you want to delete this priority class? This action cannot be undone.
                    </p>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{priorityClass.value}</p>
                      </div>
                      <div>
                        <span className="text-xs text-gray-500 dark:text-gray-400">Cluster</span>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{priorityClass.clusterName}</p>
                      </div>
                      {priorityClass.globalDefault && (
                        <div>
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                            This is the global default priority class
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                      <p className="text-sm text-red-600 dark:text-red-400">
                        <strong>Warning:</strong> Deleting this priority class may affect existing pods that reference it. 
                        {priorityClass.globalDefault && ' This is the global default priority class, which will affect all pods without an explicit priority class specified.'}
                      </p>
                    </div>
                  </div>
                </div>
