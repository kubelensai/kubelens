...
import { deleteStorageClass } from '@/services/api'
import { notifyResourceAction } from '@/utils/notifications'

interface DeleteStorageClassModalProps {
  storageClass: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function DeleteStorageClassModal({
  storageClass,
  isOpen,
  onClose,
  onSuccess,
}: DeleteStorageClassModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteStorageClass(
        storageClass.clusterName,
        storageClass.metadata.name
      )

      notifyResourceAction.deleted('Storage Class', storageClass.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to delete Storage Class'
      setError(errorMsg)
      notifyResourceAction.failed('delete', 'Storage Class', storageClass.metadata.name, errorMsg)
    } finally {
      setIsDeleting(false)
    }
  }
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="p-6">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Are you sure you want to delete this storage class? This action cannot be undone.
                    <span className="block mt-2 font-medium text-red-600 dark:text-red-400">
                      Warning: PVCs using this storage class may be affected!
                    </span>
                  </p>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white truncate max-w-[200px]" title={storageClass.provisioner}>
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500 dark:text-gray-400">Volume Binding Mode:</span>
                      <span className="font-medium text-gray-900 dark:text-white">
