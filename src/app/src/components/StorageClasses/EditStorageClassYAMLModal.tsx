...
interface EditStorageClassYAMLModalProps {
  storageClass: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditStorageClassYAMLModal({
  storageClass,
  isOpen,
  onClose,
  onSuccess,
}: EditStorageClassYAMLModalProps) {
  const [scYaml, setSCYaml] = useState('')
  const [error, setError] = useState('')
  const [isLoading, setIsLoading] = useState(true)
  const [isSaving, setIsSaving] = useState(false)

  useEffect(() => {
    if (isOpen && storageClass) {
      fetchSCYaml()
    }
  }, [isOpen, storageClass])

  const fetchSCYaml = async () => {
    setIsLoading(true)
    try {
      const response = await api.get(
        `/clusters/${storageClass.clusterName}/storageclasses/${storageClass.metadata.name}`
      )
      const cleaned = cleanKubernetesManifest(response.data, 'storageclass')
      setSCYaml(yaml.dump(cleaned, { indent: 2, lineWidth: -1 }))
      setError('')
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to fetch Storage Class')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const parsed = yaml.load(scYaml) as any
      ...
      notifyResourceAction.updated('Storage Class', storageClass.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to update Storage Class'
      setError(errorMsg)
      notifyResourceAction.failed('update', 'Storage Class', storageClass.metadata.name, errorMsg)
    } finally {
      setIsSaving(false)
    }
  }
