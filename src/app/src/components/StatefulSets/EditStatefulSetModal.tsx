...
interface EditStatefulSetModalProps {
  statefulset: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditStatefulSetModal({
  statefulset,
  isOpen,
  onClose,
  onSuccess,
}: EditStatefulSetModalProps) {
  const [statefulsetYaml, setStatefulSetYaml] = useState<string>('')
  ...
  const fetchStatefulSetYaml = async () => {
    setIsLoading(true)
    setError('')
    try {
      ...
      const cleanedManifest = cleanKubernetesManifest(response.data, 'statefulset')
      ...
      setStatefulSetYaml(yamlStr)
    } catch (err) {
      console.error('Failed to fetch statefulset:', err)
      setError('Failed to load statefulset data')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      // Parse YAML to JSON
      const parsedStatefulSet = yaml.load(statefulsetYaml)
      ...
      console.error('Failed to update statefulset:', err)
      if (err.name === 'YAMLException') {
        setError(`YAML Syntax Error: ${err.message}`)
      } else {
        setError(
          err.response?.data?.error || 'Failed to update statefulset. Please check your YAML syntax.'
        )
      }
    } finally {
      setIsSaving(false)
              ...
              <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Edit StatefulSet: {statefulset.metadata?.name}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
