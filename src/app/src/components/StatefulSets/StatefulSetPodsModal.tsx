...
interface StatefulSetPodsModalProps {
  statefulset: any
  isOpen: boolean
  onClose: () => void
}
...
export default function StatefulSetPodsModal({
  statefulset,
  isOpen,
  onClose,
}: StatefulSetPodsModalProps) {
  ...
  const { data: allPods, isLoading } = useQuery({
    queryKey: ['statefulset-pods', statefulset?.clusterName, statefulset?.metadata?.namespace, statefulset?.metadata?.name],
    queryFn: async () => {
      if (!statefulset) return []
      const pods = await getPods(statefulset.clusterName, statefulset.metadata.namespace)
      return pods
    },
    enabled: isOpen && !!statefulset,
  })

  // Filter pods that belong to this statefulset
  useEffect(() => {
    if (allPods && statefulset) {
      const statefulsetSelector = statefulset.spec?.selector?.matchLabels || {}
      const filteredPods = allPods.filter((pod: any) => {
        const podLabels = pod.metadata?.labels || {}
        // Check if all statefulset selector labels match pod labels
        return Object.entries(statefulsetSelector).every(
  ...
  useEffect(() => {
    if (isOpen && pods.length > 0 && statefulset) {
      fetchPodMetrics()
    }
      ...
      const metricsPromises = pods.map(async (pod) => {
        try {
          const response = await api.get(
            `/clusters/${statefulset.clusterName}/namespaces/${pod.metadata.namespace}/pods/${pod.metadata.name}/metrics`
          )
          return {
  ...
  const handlePodClick = (_pod: any) => {
    navigate(`/clusters/${statefulset.clusterName}/namespaces/${statefulset.metadata.namespace}/pods`)
    onClose()
  }
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                      {statefulset.metadata.name} • {statefulset.metadata.namespace}
                    </p>
                  </div>
                  <button
                    onClick={onClose}
                  ...
                  {isLoading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        This statefulset has no running pods
                      </p>
                    </div>
                  )}
                </div>
