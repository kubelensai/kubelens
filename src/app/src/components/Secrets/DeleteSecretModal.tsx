...
export default function DeleteSecretModal({ secret, isOpen, onClose, onSuccess }: DeleteSecretModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notifyResourceAction.deleted('Secret', secret.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to delete secret'
      setError(errorMsg)
      notifyResourceAction.failed('delete', 'Secret', secret.metadata.name, errorMsg)
    } finally {
      setIsDeleting(false)
    }
  }
                ...
                <div className="p-6">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">
                        {secret.data ? Object.keys(secret.data).length : 0} keys
                      </span>
                    </div>
                  </div>
