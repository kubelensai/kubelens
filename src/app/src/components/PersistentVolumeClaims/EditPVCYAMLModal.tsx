...
interface EditPVCYAMLModalProps {
  pvc: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditPVCYAMLModal({
  pvc,
  isOpen,
  onClose,
  onSuccess,
}: EditPVCYAMLModalProps) {
  const [pvcYaml, setPvcYaml] = useState('')
  ...
  const fetchPVCYaml = async () => {
    setIsLoading(true)
    try {
      const response = await api.get(
        `/clusters/${pvc.clusterName}/namespaces/${pvc.metadata.namespace}/persistentvolumeclaims/${pvc.metadata.name}`
      )
      const cleaned = cleanKubernetesManifest(response.data, 'persistentvolumeclaim')
      setPvcYaml(yaml.dump(cleaned, { indent: 2, lineWidth: -1 }))
      setError('')
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to fetch PVC')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const parsed = yaml.load(pvcYaml) as any
      ...
      notifyResourceAction.updated('PVC', pvc.metadata.name)
      onSuccess()
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || err.message || 'Failed to update PVC'
      setError(errorMsg)
      notifyResourceAction.failed('update', 'PVC', pvc.metadata.name, errorMsg)
    } finally {
      setIsSaving(false)
    }
  }
