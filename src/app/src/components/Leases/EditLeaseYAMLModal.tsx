...
export default function EditLeaseYAMLModal({
  lease,
  isOpen,
  onClose,
  onSuccess,
}: EditLeaseYAMLModalProps) {
  ...
  useEffect(() => {
    if (isOpen && lease) {
      ...
      getLease(lease.clusterName, lease.metadata.namespace, lease.metadata.name)
        .then((data) => {
          const cleanedManifest = cleanKubernetesManifest(data, 'lease')
        ...
        .catch((err) => {
          ...
          setError('Failed to load lease YAML')
          setIsLoading(false)
        })
    }
  }, [isOpen, lease])
  ...
  const handleSave = async () => {
    try {
      ...
      await api.put(
        `/clusters/${lease.clusterName}/namespaces/${lease.metadata.namespace}/leases/${lease.metadata.name}`,
        parsedYaml
      )
      ...
      setError(err.response?.data?.error || err.message || 'Failed to update lease')
    } finally {
      setIsSaving(false)
    }
  }
