...
import { getValidatingWebhookConfiguration } from '@/services/api'
import yaml from 'js-yaml'
import { cleanKubernetesManifest } from '@/utils/kubernetes'
import api from '@/services/api'
import YamlEditor from '@/components/shared/YamlEditor'

interface EditValidatingWebhookYAMLModalProps {
  webhook: any
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditValidatingWebhookYAMLModal({
  webhook,
  isOpen,
  onClose,
  onSuccess,
}: EditValidatingWebhookYAMLModalProps) {
  ...
  useEffect(() => {
    if (isOpen && webhook) {
      ...
      getValidatingWebhookConfiguration(webhook.clusterName, webhook.metadata.name)
        .then((data) => {
          const cleanedManifest = cleanKubernetesManifest(data, 'validatingwebhookconfiguration')
  ...
  const handleSave = async () => {
    try {
      ...
      await api.put(
        `/clusters/${webhook.clusterName}/validatingwebhookconfigurations/${webhook.metadata.name}`,
        parsedYaml
      )
