...
export default function EditPodModal({ isOpen, onClose, pod, clusterName, onSuccess }: EditPodModalProps) {
  ...
  const fetchPodYaml = async () => {
    setIsLoading(true)
    setError('')
    try {
      const response = await api.get(
        `/clusters/${clusterName}/namespaces/${pod.metadata.namespace}/pods/${pod.metadata.name}`
      )
      // Clean and reorder the manifest
      const cleanedManifest = cleanKubernetesManifest(response.data, 'pod')
  ...
  const handleSave = async () => {
    ...
    try {
      // Parse YAML to JSON
      const parsedPod = yaml.load(podYaml)
      ...
      if (err.name === 'YAMLException') {
        ...
        setError(err.response?.data?.error || 'Failed to update pod. Please check your YAML syntax.')
      }
    } finally {
      setIsSaving(false)
    }
              ...
              <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white dark:bg-[#1a1f2e] shadow-2xl transition-all">
                ...
                <div className="px-6 py-3 bg-yellow-50 dark:bg-yellow-900/20 border-b border-yellow-200 dark:border-yellow-800">
                  <p className="text-sm text-yellow-800 dark:text-yellow-300">
                    <strong>Warning:</strong> Editing a pod directly is not recommended. Most pod fields are immutable. Consider deleting and recreating the pod or editing the controller (Deployment, StatefulSet, etc.) instead.
                  </p>
                </div>
