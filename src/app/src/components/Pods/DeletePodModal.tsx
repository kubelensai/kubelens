...
export default function DeletePodModal({
  pod,
  isOpen,
  onClose,
  onSuccess,
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      await api.delete(
        `/clusters/${pod.clusterName}/namespaces/${pod.metadata.namespace}/pods/${pod.metadata.name}`
      )
    },
    onSuccess: () => {
      notifyResourceAction.deleted('Pod', pod.metadata.name)
      onSuccess()
    },
    onError: (err: any) => {
      console.error('Failed to delete pod:', err)
      notifyResourceAction.failed('delete', 'Pod', pod.metadata.name, err.response?.data?.error || err.message)
    },
  })
                      ...
                      <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mb-4">
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            <span className="text-gray-600 dark:text-gray-400">Pod:</span>
                          ...
                          <div className="ml-3">
                            ...
                            <div className="mt-2 text-sm text-red-700 dark:text-red-400">
                              <ul className="list-disc list-inside space-y-1">
                                <li>Pod will immediately terminate</li>
                                <li>Running containers will be stopped</li>
                                <li>If managed by a controller, it will be recreated</li>
                              </ul>
                            </div>
                          </div>
                        </div>
                ...
                {deleteMutation.isError && (
                  <div className="mt-4 rounded-md bg-red-50 dark:bg-red-900/20 p-3">
                    <p className="text-sm text-red-800 dark:text-red-300">
                      Failed to delete pod. Please try again.
                    </p>
                  </div>
                )}
              </Dialog.Panel>
