...
interface DaemonSetPodsModalProps {
  daemonset: any
  isOpen: boolean
  onClose: () => void
}
...
export default function DaemonSetPodsModal({
  daemonset,
  isOpen,
  onClose,
}: DaemonSetPodsModalProps) {
  ...
  const { data: allPods, isLoading } = useQuery({
    queryKey: ['daemonset-pods', daemonset?.clusterName, daemonset?.metadata?.namespace, daemonset?.metadata?.name],
    queryFn: async () => {
      if (!daemonset) return []
      const pods = await getPods(daemonset.clusterName, daemonset.metadata.namespace)
      return pods
    },
    enabled: isOpen && !!daemonset,
  })

  // Filter pods that belong to this daemonset
  useEffect(() => {
    if (allPods && daemonset) {
      const daemonsetSelector = daemonset.spec?.selector?.matchLabels || {}
      const filteredPods = allPods.filter((pod: any) => {
        const podLabels = pod.metadata?.labels || {}
        // Check if all daemonset selector labels match pod labels
        return Object.entries(daemonsetSelector).every(
  ...
  useEffect(() => {
    if (isOpen && pods.length > 0 && daemonset) {
      fetchPodMetrics()
    }
      ...
      const metricsPromises = pods.map(async (pod) => {
        try {
          const response = await api.get(
            `/clusters/${daemonset.clusterName}/namespaces/${pod.metadata.namespace}/pods/${pod.metadata.name}/metrics`
          )
          return {
  ...
  const handlePodClick = (_pod: any) => {
    navigate(`/clusters/${daemonset.clusterName}/namespaces/${daemonset.metadata.namespace}/pods`)
    onClose()
  }
                ...
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                      {daemonset.metadata.name} • {daemonset.metadata.namespace}
                    </p>
                  </div>
                  <button
                    onClick={onClose}
                  ...
                  {isLoading ? (
                          ...
                          {pods.map((pod: any) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="py-3 px-4">
                                  {metrics && metrics.cpuMillicores > 0 ? (
                                    renderResourceBar(
                                    ...
                                    <div className="text-xs text-gray-400 dark:text-gray-500 text-right">
                                      {metricsLoading ? 'Loading...' : 'N/A'}
                                    </div>
                                  )}
                                </td>
                                <td className="py-3 px-4">
                                  {metrics && metrics.memoryBytes > 0 ? (
                                    renderResourceBar(
                                      metrics.memoryBytes,
                                      limits.memoryLimit,
                                      formatBytes
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        This daemonset has no running pods.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">
                      Total Pods: <span className="font-semibold text-gray-900 dark:text-white">{pods.length}</span>
                    </span>
                    <span className="text-gray-600 dark:text-gray-400">
                      Click on a pod to view all pods in the namespace
                    </span>
                  </div>
                </div>
              </Dialog.Panel>
