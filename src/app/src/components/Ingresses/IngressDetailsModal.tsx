...
export default function IngressDetailsModal({ ingress, isOpen, onClose, onSuccess }: IngressDetailsModalProps) {
  const [dataFields, setDataFields] = useState<Record<string, string>>({})
  const [isEditing, setIsEditing] = useState(false)
  const [isSaving, setIsSaving] = useState(false)

  // Initialize data fields when ingress changes
  useEffect(() => {
    if (ingress?.data) {
      setDataFields({ ...ingress.data })
    }
  }, [ingress])
